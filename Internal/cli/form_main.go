...
func (m *FormMain) initLists(width, height int) {
	...
	for _, db := range arrDB {
		...
		listItem = append(listItem, Task{status: constants.Blocking, dbName: db.Name,
			title: fmt.Sprintf("%s", title), description: description, uid: db.UID})
	}
	...
	for _, service := range storageService {
		...
		listServersItem = append(listServersItem, Task{status: constants.Services, dbName: "",
			title: service.NameServer, description: service.NameService, uid: service.UID})
	}
		...
		default:
			...
			for kk, v := range items {
				for _, val := range storageDB {
					...
					items[kk] = Task{status: constants.Blocking, dbName: val.Name, title: fmt.Sprintf("%s", title),
						description: fmt.Sprintf("Srvr=\"%s\";Ref=\"%s\";", val.Server, val.NameOnServer),
						uid:         val.UID}
				}
			}
...
func (m *FormMain) executeEnterFormMain() (tea.Model, tea.Cmd) {
	if m.focused == 0 {
		uid := m.lists[m.focused].SelectedItem().(Task).uid
		...
		model := client.Models[constants.FormPropertyDB].(*FormBlock)
